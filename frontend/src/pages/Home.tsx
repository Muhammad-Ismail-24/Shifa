  ...
  const submit = useCallback(
    async (rawText: string) => {
      ...
      try {
        ...
        const response = await analyze({ urdu_text: text, latitude, longitude, history, language });
    ...
    <>
      ...
      <div className="relative z-10 flex h-screen h-[100dvh] flex-col text-black">
        <ShifaNav menuOpen={menuOpen} onOpenMenu={() => setMenuOpen(true)}>
          {/* Header controls — language toggle and auth */}
          <div className="flex items-center gap-4">
            <div className="rounded-full border border-white/20 bg-white/10 backdrop-blur-md p-0.5 flex items-center shadow-sm">
              <button
                type="button"
                onClick={() => setLanguage('en')}
                className={
                  language === 'en'
                    ? 'bg-black/80 text-white rounded-full px-3 py-1 text-xs font-medium transition'
                    : 'text-black/60 rounded-full px-3 py-1 text-xs font-medium cursor-pointer hover:text-black transition'
                }
              >
                EN
              </button>
            ...
            {user ? (
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full border border-white/20 bg-white/10 text-sm font-medium text-black/70 backdrop-blur-md shadow-sm">
